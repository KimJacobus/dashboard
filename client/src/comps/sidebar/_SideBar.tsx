...
export const _SideBar = ({ fetchPeople }: { fetchPeople: ({ input }: inputProps) => void }) => {
  ...
  const pathHandler = (buttonText: string) => {
    switch (buttonText) {
      case 'Dashboard':
      case 'Dash':
      ...
      case 'Projects':
      case 'Jobs':
        navigate(routerBasePath + 'projects')
        break
      case 'Available':
        navigate(routerBasePath + 'available')
    }
  }
  ...
  return (
    <div className="sidebar-grid-wrapper col-span-2 col-start-2 row-span-6 hidden rounded-lg bg-slate-50 lg:mt-4 xl:block">
      <div className="sidebar-flexbox flex flex-col font-thin">
        <h2 className="h-28 cursor-pointer ps-4 pt-7 text-6xl" onClick={() => pathHandler('Dash')}>
          Dash
        </h2>
